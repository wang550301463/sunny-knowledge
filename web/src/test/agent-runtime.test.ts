...
describe("authorized Agent UI event boundary",()=>{
 it("removes every content field when a run is redacted",()=>{const hidden=readableRun({...run,content_hidden:true,question:"PRIVATE",answer:{facts:[],inferences:[],gaps:["PRIVATE"]}});expect(hidden.content_hidden).toBe(true);expect(JSON.stringify(hidden)).not.toContain("PRIVATE");});
