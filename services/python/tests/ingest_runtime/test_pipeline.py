...
async def setup_pipeline(store,files=None):
    db,auth,box=store
    source=await create(store,SourceCreate(name='repo',space_id='space',kind='git',config={'url':'https://host/repo','ref':'HEAD'}))
...
async def test_real_parser_pipeline_registers_exact_source_before_publishing(store):
    ...
    assert len(knowledge.snapshots)==2
    ...
    db,auth,box=store
    async with db.session() as s: assert (await s.get(Task,task['id'])).status=='succeeded'
...
async def test_cas_conflict_becomes_review_needed_without_force_retry(store):
    ...
    await finish(pipeline,task['id'])
    db,auth,box=store
    async with db.session() as s: assert (await s.get(Task,task['id'])).status=='review_needed'
...
async def test_revoked_worker_stops_before_canonical_write(store):
    ...
    async with db.session() as s: assert (await s.get(Task,task['id'])).status=='failed'