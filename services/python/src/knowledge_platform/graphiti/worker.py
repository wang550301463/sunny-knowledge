...
from .models import Audit, Task, initialize, now
from .pipeline import TERMINAL, Pipeline
...
class Activities:
    ...
    async def advance(self, task_id: str) -> bool:
        ...
        except Exception:  # noqa: BLE001 -- fail closed without leaking third-party payloads
            # No source content, SQL parameters, upstream exception text, or tokens in history.
            raise ApplicationError(
                "Ingest dependency operation failed", type="IngestDependencyFailure"
            ) from None
        ...
        try:
            async with self.pipeline.db.session() as session, session.begin():
                ...
                if task and task.status not in TERMINAL:
                    task.status, task.stage, task.error_code, task.updated_at = (
                        "failed",
                        "failed",
                        "retry_exhausted",
                        now(),
                    ...
                    session.add(
                        Audit(
                            ...
                            task_id=task.id,
                            actor_id=task.checkpoint.get("worker_actor_id")
                            or task.checkpoint["scheduled_worker_id"],
                            action="task.failed",
                            detail={
                                "error_code": "retry_exhausted",
                                "identity_source": "last_successful_worker_authorization",
                            },
                        )
                    )
        except Exception:  # noqa: BLE001 -- fail closed without leaking third-party payloads
            raise ApplicationError(
                "Ingest task checkpoint unavailable", type="IngestDependencyFailure"
            ) from None
...
async def main():
    ...
    pipeline = Pipeline(
        ...
        machine,
        internal,
        SecretBox(config.ingest_encryption_key),
        S3Store.from_settings(config),
    )
    client = await Client.connect(
        config.ingest_temporal_address, namespace=config.ingest_temporal_namespace
    )
    activities = Activities(pipeline)
    dispatcher = TemporalDispatcher(db, client, config.ingest_temporal_task_queue)
    worker = Worker(
        client,
        task_queue=config.ingest_temporal_task_queue,
        workflows=[IngestWorkflow],
        activities=[activities.advance, activities.fail],
    ...
    try:
        async with worker:
            while True:
                ...
                except Exception:  # noqa: BLE001 -- safe boundary for dependency failures
                    logging.getLogger(__name__).warning("Ingest task dispatch unavailable")
